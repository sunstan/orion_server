...
export class PublicResolver {
  ...
  async signIn(
    ...
    const session = await this.sessionService.repository.createSession({
      user,
    });
    ...
    const duration = parseInt(
      this.configService.get<string>('AUTH_COOKIE_DURATION'),
      10,
    );
  ...
  @Mutation(() => Boolean)
  async signOut(@GqlResponse() response: Response): Promise<boolean> {
