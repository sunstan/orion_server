...
export class BlockResolver {
    ...
    async toggleBlock(
        ...
        if (receiver.roles.includes(UserRoles.ADMIN)) throw new ForbiddenException('Admin can\'t be blocked');
        if (receiver.roles.includes(UserRoles.TEACHER)) throw new ForbiddenException('Teacher can\'t be blocked');

        const where = {emitter: {id: emitter.id}, receiver: {id: receiver.id}};
        const block = await this.blockService.repository.findOne({where});

        return block
            ? !!await this.blockService.repository.delete(block.id)
            : !!await this.blockService.repository.createBlock({emitter, receiver});
    }
}
