    ...
    async createReport (
        ...
        switch (type) {
            case ReportTypes.POST: {
                ...
                return await this.reportService.repository.createReport({referenceId, emitter, type});
            }

            case ReportTypes.PROFILE: {
                const profile = await this.profileService.repository.findOne(referenceId);
